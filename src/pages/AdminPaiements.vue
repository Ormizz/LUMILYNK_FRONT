<template>
  <AdminGuard>
    <div class="min-h-full">
      <NavAdmin />
      
      <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl py-4 px-4 sm:py-6 sm:px-6 lg:px-8">
          <h1 class="text-lg font-semibold tracking-tight text-gray-300" style="font-size: 50px;">
            Paiements étudiants
          </h1>
        </div>
      </header>

      <main class="py-4 sm:py-6 lg:py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          
          <!-- Section des filtres -->
          <div class="bg-white shadow rounded-lg p-4 sm:p-6 mb-4 sm:mb-6">
            <h2 class="text-base sm:text-lg font-medium text-gray-900 mb-3 sm:mb-4">Filtres</h2>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <!-- Filtre par nom d'étudiant -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Nom de l'étudiant
                </label>
                <input
                  v-model="filters.nom"
                  type="text"
                  placeholder="Nom de l'étudiant..."
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>

              <!-- Filtre par référence -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Référence
                </label>
                <input
                  v-model="filters.reference"
                  type="text"
                  placeholder="Référence du paiement..."
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>

              <!-- Filtre par statut -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Statut
                </label>
                <select
                  v-model="filters.status"
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="">Tous les statuts</option>
                  <option value="En attente">En attente</option>
                  <option value="valide">Validé</option>
                  <option value="refuse">Refusé</option>
                </select>
              </div>

              <!-- Filtre par type -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Type de paiement
                </label>
                <select
                  v-model="filters.type"
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="">Tous les types</option>
                  <option
                    v-for="type in uniqueTypes"
                    :key="type"
                    :value="type"
                  >
                    {{ type }}
                  </option>
                </select>
              </div>
            </div>

            <!-- Filtres supplémentaires -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
              <!-- Filtre par Montant minimum -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Montant minimum (FCFA)
                </label>
                <input
                  v-model.number="filters.amountMin"
                  type="number"
                  placeholder="0"
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>

              <!-- Filtre par Montant maximum -->
              <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                  Montant maximum (FCFA)
                </label>
                <input
                  v-model.number="filters.amountMax"
                  type="number"
                  placeholder="1000000"
                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md text-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>
            </div>

            <!-- Boutons d'action pour les filtres -->
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mt-4">
              <button
                @click="clearFilters"
                class="w-full sm:w-auto px-4 py-2 text-sm text-gray-600 bg-gray-100 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-colors duration-200"
              >
                Réinitialiser les filtres
              </button>
              <div class="text-xs sm:text-sm text-gray-600 text-center sm:text-right">
                {{ filteredPaiements.length }} paiement(s) trouvé(s) -
                Total: {{ totalamount.toLocaleString() }} FCFA
              </div>
            </div>
          </div>

          <!-- Tableau des paiements - Desktop -->
          <div class="hidden lg:block overflow-x-auto bg-white shadow border rounded-lg">
            <table class="min-w-full text-left text-sm">
              <thead class="bg-gray-100 text-gray-600 uppercase text-xs font-medium">
                <tr>
                  <th class="p-4">Référence</th>
                  <th class="p-4">Nom étudiant</th>
                  <th class="p-4">Montant</th>
                  <th class="p-4">Type</th>
                  <th class="p-4">Statut</th>
                  <th class="p-4">Date</th>
                  <th class="p-4">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="paiement in paginatedPaiements"
                  :key="paiement.id"
                  class="border-t hover:bg-gray-50 transition-colors duration-150"
                >
                  <td class="p-4 font-semibold text-indigo-700">
                    {{ paiement.reference }}
                  </td>
                  <td class="p-4 font-medium">
                    {{ paiement.nom }}
                  </td>
                  <td class="p-4 font-semibold">
                    {{ formatamount(paiement.amount) }}
                  </td>
                  <td class="p-4">
                    <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                      {{ paiement.type }}
                    </span>
                  </td>
                  <td class="p-4">
                    <span 
                      :class="getStatusClass(paiement.status)"
                      class="px-2 py-1 rounded-full text-xs font-medium"
                    >
                      {{ getStatusLabel(paiement.status) }}
                    </span>
                  </td>
                  <td class="p-4">
                    {{ formatDate(paiement.date) }}
                  </td>
                  <td class="p-4">
                    <div class="flex space-x-2">
                      <a
                        :href="`${BASE_URL}${paiement.proof_url}`"
                        target="_blank"
                        class="text-indigo-600 hover:text-indigo-800 hover:underline text-sm transition-colors duration-200"
                      >
                        Voir
                      </a>
                      <button
                        @click="updatePaiementStatus(paiement.id, 'valide')"
                        class="text-green-600 hover:text-green-800 hover:underline text-sm transition-colors duration-200"
                        style="margin-right: 10px;"
                      >
                        Valider
                      </button>
                      <button 
                        @click="updatePaiementStatus(paiement.id, 'refuse')"
                        class="text-red-600 hover:text-red-800 hover:underline text-sm transition-colors duration-200"
                      >
                        Rejeter
                      </button>
                    </div>
                  </td>
                </tr>
                <tr v-if="filteredPaiements.length === 0">
                  <td colspan="7" class="p-8 text-center text-gray-500">
                    <div class="flex flex-col items-center">
                      <svg class="w-12 h-12 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
                      </svg>
                      Aucun paiement trouvé avec ces filtres.
                    </div>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>

          <!-- Cards View - Mobile & Tablet -->
          <div class="lg:hidden space-y-4">
            <div
              v-for="paiement in paginatedPaiements"
              :key="paiement.id"
              class="bg-white shadow rounded-lg p-4 border border-gray-200"
            >
              <!-- Header with Reference and Status -->
              <div class="flex justify-between items-start mb-3">
                <div class="flex-1">
                  <p class="text-xs text-gray-500 mb-1">Référence</p>
                  <p class="font-semibold text-indigo-700 text-sm">{{ paiement.reference }}</p>
                </div>
                <span 
                  :class="getStatusClass(paiement.status)"
                  class="px-2 py-1 rounded-full text-xs font-medium ml-2"
                >
                  {{ getStatusLabel(paiement.status) }}
                </span>
              </div>

              <!-- Student Name -->
              <div class="mb-3">
                <p class="text-xs text-gray-500 mb-1">Nom étudiant</p>
                <p class="font-medium text-gray-900 text-sm">{{ paiement.nom }}</p>
              </div>

              <!-- Amount and Type -->
              <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <p class="text-xs text-gray-500 mb-1">Montant</p>
                  <p class="font-semibold text-gray-900 text-sm">{{ formatamount(paiement.amount) }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-500 mb-1">Type</p>
                  <span class="inline-block px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                    {{ paiement.type }}
                  </span>
                </div>
              </div>

              <!-- Date -->
              <div class="mb-3">
                <p class="text-xs text-gray-500 mb-1">Date</p>
                <p class="text-gray-900 text-xs">{{ formatDate(paiement.date) }}</p>
              </div>

              <!-- Actions -->
              <div class="flex flex-wrap gap-2 pt-3 border-t border-gray-200">
                <a
                  :href="`${BASE_URL}${paiement.proof_url}`"
                  target="_blank"
                  class="flex-1 text-center px-3 py-2 bg-indigo-50 text-indigo-600 rounded-md text-xs font-medium hover:bg-indigo-100 transition-colors duration-200"
                >
                  Voir preuve
                </a>
                <button
                  @click="updatePaiementStatus(paiement.id, 'valide')"
                  class="flex-1 px-3 py-2 bg-green-50 text-green-600 rounded-md text-xs font-medium hover:bg-green-100 transition-colors duration-200"
                >
                  Valider
                </button>
                <button 
                  @click="updatePaiementStatus(paiement.id, 'refuse')"
                  class="flex-1 px-3 py-2 bg-red-50 text-red-600 rounded-md text-xs font-medium hover:bg-red-100 transition-colors duration-200"
                >
                  Rejeter
                </button>
              </div>
            </div>

            <!-- Empty State -->
            <div v-if="filteredPaiements.length === 0" class="bg-white shadow rounded-lg p-8 text-center">
              <svg class="w-12 h-12 text-gray-300 mb-2 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
              </svg>
              <p class="text-gray-500 text-sm">Aucun paiement trouvé avec ces filtres.</p>
            </div>
          </div>

          <!-- Pagination -->
          <div
            v-if="filteredPaiements.length > itemsPerPage"
            class="mt-4 sm:mt-6 px-4 py-3 bg-white shadow rounded-lg border border-gray-200"
          >
            <div class="flex items-center justify-between">
              <!-- Mobile pagination -->
              <div class="flex-1 flex justify-between sm:hidden">
                <button
                  @click="previousPage"
                  :disabled="currentPage === 1"
                  class="relative inline-flex items-center px-3 py-2 border border-gray-300 text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                >
                  Précédent
                </button>
                <span class="text-xs text-gray-700 flex items-center">
                  Page {{ currentPage }} / {{ totalPages }}
                </span>
                <button 
                  @click="nextPage" 
                  :disabled="currentPage === totalPages"
                  class="relative inline-flex items-center px-3 py-2 border border-gray-300 text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                >
                  Suivant
                </button>
              </div>
              
              <!-- Desktop pagination -->
              <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p class="text-xs sm:text-sm text-gray-700">
                    Affichage de 
                    <span class="font-medium">{{ (currentPage - 1) * itemsPerPage + 1 }}</span> 
                    à 
                    <span class="font-medium">{{ Math.min(currentPage * itemsPerPage, filteredPaiements.length) }}</span> 
                    sur 
                    <span class="font-medium">{{ filteredPaiements.length }}</span> 
                    résultats
                  </p>
                </div>
                <div>
                  <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                    <button 
                      @click="previousPage" 
                      :disabled="currentPage === 1"
                      class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                    >
                      <span class="sr-only">Précédent</span>
                      <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                      </svg>
                    </button>
                    
                    <button
                      v-for="page in visiblePages"
                      :key="page"
                      @click="goToPage(page)"
                      :class="[
                        page === currentPage
                          ? 'bg-indigo-50 border-indigo-500 text-indigo-600'
                          : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50',
                        'relative inline-flex items-center px-4 py-2 border text-sm font-medium transition-colors duration-200'
                      ]"
                    >
                      {{ page }}
                    </button>
                    
                    <button
                      @click="nextPage"
                      :disabled="currentPage === totalPages"
                      class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                    >
                      <span class="sr-only">Suivant</span>
                      <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                      </svg>
                    </button>
                  </nav>
                </div>
              </div>
            </div>
          </div>
        </div>
      </main>
    </div>
  </AdminGuard>
</template>

<script setup>
import { ref, onMounted, computed, watch } from 'vue';
import NavAdmin from '@/components/Dashboard/Admin/NavAdmin.vue';
import AdminGuard from '@/components/AdminGuard.vue';
import { BASE_URL } from '@/config';

// ===========================
// REACTIVE DATA
// ===========================
const paiements = ref([]);
const currentPage = ref(1);
const itemsPerPage = ref(10);

// Filtres
const filters = ref({
  nom: '',
  reference: '',
  status: '',
  type: '',
  amountMin: null,
  amountMax: null
});

// ===========================
// API CALLS
// ===========================
const fetchPaiements = async () => {
  try {
    const response = await fetch(`${BASE_URL}/api/payments`);
    if (!response.ok) {
      throw new Error('Erreur lors du chargement');
    }
    paiements.value = await response.json();
  } catch (error) {
    console.error('Erreur fetch paiements:', error);
  }
};

const updatePaiementStatus = async (id, newStatus) => {
  try {
    const response = await fetch(`${BASE_URL}/api/payments/single/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ status: newStatus }),
    });

    if (!response.ok) {
      throw new Error('Erreur lors de la mise à jour');
    }

    const result = await response.json();
    console.log('Statut mis à jour:', result);

    // Recharge les paiements après mise à jour
    await fetchPaiements();
    // window.location.reload();
  } catch (error) {
    console.error('Erreur update paiement:', error);
    alert('Échec de la mise à jour du statut.');
  }
};

// ===========================
// COMPUTED PROPERTIES
// ===========================
const uniqueTypes = computed(() => {
  const types = Array.from(new Set(paiements.value.map(p => p.type)));
  return types.sort();
});

const filteredPaiements = computed(() => {
  return paiements.value.filter(paiement => {
    const matchNom = !filters.value.nom || 
      paiement.nom.toLowerCase().includes(filters.value.nom.toLowerCase());
    
    const matchReference = !filters.value.reference || 
      paiement.reference.toLowerCase().includes(filters.value.reference.toLowerCase());
    
    const matchStatus = !filters.value.status || 
      paiement.status === filters.value.status;
    
    const matchType = !filters.value.type || 
      paiement.type === filters.value.type;

    const matchamountMin = !filters.value.amountMin || 
      parseFloat(paiement.amount) >= filters.value.amountMin;

    const matchamountMax = !filters.value.amountMax || 
      parseFloat(paiement.amount) <= filters.value.amountMax;

    return matchNom && matchReference && matchStatus && matchType && matchamountMin && matchamountMax;
  });
});

const totalPages = computed(() => {
  return Math.ceil(filteredPaiements.value.length / itemsPerPage.value);
});

const paginatedPaiements = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  const end = start + itemsPerPage.value;
  return filteredPaiements.value.slice(start, end);
});

const visiblePages = computed(() => {
  const pages = [];
  const maxVisible = 5;
  let start = Math.max(1, currentPage.value - Math.floor(maxVisible / 2));
  let end = Math.min(totalPages.value, start + maxVisible - 1);
  start = Math.max(1, end - maxVisible + 1);

  for (let i = start; i <= end; i++) {
    pages.push(i);
  }
  return pages;
});

const totalamount = computed(() => {
  return filteredPaiements.value.reduce((total, paiement) => {
    if (paiement.status === 'valide') { 
      return total + parseFloat(paiement.amount || 0);
    }
    return total;
  }, 0);
});

// ===========================
// METHODS
// ===========================
const clearFilters = () => {
  filters.value = {
    nom: '',
    reference: '',
    status: '',
    type: '',
    amountMin: null,
    amountMax: null
  };
  currentPage.value = 1;
};

const getStatusClass = (status) => {
  switch (status) {
    case 'valide':
      return 'bg-green-100 text-green-800';
    case 'refuse':
      return 'bg-red-100 text-red-800';
    case 'En attente':
    default:
      return 'bg-yellow-100 text-yellow-800';
  }
};

const getStatusLabel = (status) => {
  switch (status) {
    case 'valide':
      return 'Validé';
    case 'refuse':
      return 'Refusé';
    case 'En attente':
    default:
      return 'En attente';
  }
};

const formatDate = (dateString) => {
  return new Date(dateString).toLocaleDateString('fr-FR', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit'
  });
};

const formatamount = (amount) => {
  return `${parseFloat(amount).toLocaleString()} FCFA`;
};

// Pagination methods
const previousPage = () => {
  if (currentPage.value > 1) {
    currentPage.value--;
  }
};

const nextPage = () => {
  if (currentPage.value < totalPages.value) {
    currentPage.value++;
  }
};

const goToPage = (page) => {
  currentPage.value = page;
};

// ===========================
// WATCHERS
// ===========================
// Reset pagination when filters change
watch(filters, () => {
  currentPage.value = 1;
}, { deep: true });

// ===========================
// LIFECYCLE
// ===========================
onMounted(() => {
  fetchPaiements();
});
</script>

<style scoped>
/* Transitions personnalisées si nécessaire */
.fade-enter-active, .fade-leave-active {
  transition: opacity 0.3s;
}
.fade-enter, .fade-leave-to {
  opacity: 0;
}
</style>